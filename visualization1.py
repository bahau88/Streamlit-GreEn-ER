...
# Create the subplots
fig_exogeneous = make_subplots(rows=2, cols=3, subplot_titles=('Number of Room', 'Dayindex', 'Occupants',
...
selected_data = st.multiselect('Select data to show', ['Consumption', 'Other', 'Heating', 'Lighting', 'All',
                                                       'Number of Room', 'Dayindex', 'Occupants',
                                                       'Temperature', 'Cloudcover', 'Visibility'],
                               default=['All'])

# Filter the data based on the user's selection
if 'All' in selected_data:
    selected_data = ['Consumption', 'Other', 'Heating', 'Lighting', 'Number of Room', 'Dayindex', 'Occupants',
                     'Temperature', 'Cloudcover', 'Visibility']
...
for data in selected_data:
    ...
    elif data == 'Number of Room':
        selected_traces.append(fig_exogeneous['data'][0])
    elif data == 'Dayindex':
        selected_traces.append(fig_exogeneous['data'][1])
    elif data == 'Occupants':
        selected_traces.append(fig_exogeneous['data'][2])
    elif data == 'Temperature':
        selected_traces.append(fig_exogeneous['data'][3])
    elif data == 'Cloudcover':
        selected_traces.append(fig_exogeneous['data'][4])
    elif data == 'Visibility':
        selected_traces.append(fig_exogeneous['data'][5])

# Update the figure with the selected traces
# Update the figure with the selected traces
fig.update(data=selected_traces)
fig_exogeneous.update(data=selected_traces)
